...
class RBTree:
    ...
    def insertNode(self, key):
        ...
        node.parent = None
        node.val = key
        node.left = self.NULL  # type: ignore
        node.right = self.NULL  # type: ignore
    ...
    def fixDelete(self, x):
        while (
            x != self.root and x.color == 0  # type: ignore
        ):  # Repeat until x reaches nodes and color of x is black
            if x == x.parent.left:  # If x is left child of its parent # type: ignore
                s = x.parent.right  # Sibling of x # type: ignore
                if s.color == 1:  # if sibling is red
                    s.color = 0  # Set its color to black
                    x.parent.color = 1  # Make its parent red # type: ignore
                    self.LR(
                        x.parent  # type: ignore
                    )  # Call for left rotate on parent of x # type: ignore
                    s = x.parent.right  # type: ignore
                # If both the child are black
                if s.left.color == 0 and s.right.color == 0:
                    s.color = 1  # Set color of s as red
                    x = x.parent  # type: ignore
                else:
                    ...
                    s.color = x.parent.color  # type: ignore
                    x.parent.color = 0  # Set parent of x as black # type: ignore
                    s.right.color = 0
                    self.LR(
                        x.parent  # type: ignore
                    )  # call left rotation on parent of x # type: ignore
                    x = self.root
            else:  # If x is right child of its parent
                s = x.parent.left  # Sibling of x # type: ignore
                if s.color == 1:  # if sibling is red
                    s.color = 0  # Set its color to black
                    x.parent.color = 1  # Make its parent red # type: ignore
                    self.RR(
                        x.parent  # type: ignore
                    )  # Call for right rotate on parent of x # type: ignore
                    s = x.parent.left  # type: ignore
                ...
                if s.right.color == 0 and s.right.color == 0:
                    ...
                    s.color = x.parent.color  # type: ignore
                    x.parent.color = 0  # type: ignore
                    s.left.color = 0
                    self.RR(x.parent)  # type: ignore
                    x = self.root
        x.color = 0  # type: ignore
    ...
    def delete_node_helper(self, node, key):
        ...
        if z.left == self.NULL:  # If left child of z is NULL
            ...
            if y.parent == z:  # If y is child of z
                ...
                y.right = z.right
                y.right.parent = y  # type: ignore

            self.__rb_transplant(z, y)
            y.left = z.left
            y.left.parent = y  # type: ignore
            y.color = z.color
